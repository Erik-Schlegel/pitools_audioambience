...
parser = CustomArgumentParser(
    description="Play audio files with specified volume adjustment, trimming, offset start, and panning in a loop"
)
parser.add_argument(
    "-c",
    "--config",
    type=str,
    required=True,
    help="Path to a configuration file is required.",
)
...
def apply_panning(audio_data, panning, is_mono):
    if panning < -1 or panning > 1:
        logging.error(
            f"Invalid panning value {panning}. It should be between -1 and 1."
        )
        return audio_data
    ...
    if is_mono:
        if panning == -1:
            ...
            audio_data = np.column_stack(
                (audio_data * left_gain, audio_data * right_gain)
            )
    else:
        left_gain = (1 - panning) / 2
        right_gain = (1 + panning) / 2
...
def play_audio(file, gain_db, trim_start, trim_end, offset_start, panning):
    ...
    try:
        ...
        logging.debug(
            f"Trimmed audio segment: start={trim_start}, end={duration - trim_end}"
        )
        ...
        with sd.OutputStream(
            samplerate=frame_rate, channels=2, callback=callback, blocksize=buffer_size
        ) as stream:
        ...
        with open(config_file, "r") as file:
            for line in file:
                ...
                if line and not line.startswith("#"):  # Skip empty lines and comments
                    file_path, gain_db, trim_start, trim_end, offset_start, panning = (
                        line.split()
                    )
                    ...
                    logging.debug(
                        f"Starting thread for file: {file_path}, gain_db: {gain_db}, trim_start: {trim_start}, trim_end: {trim_end}, offset_start: {offset_start}, panning: {panning}"
                    )
                    t = threading.Thread(
                        target=play_audio,
                        args=(
